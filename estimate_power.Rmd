...
# Parameters
Nsim  <- 5000       # Number of simulations for null and shift
alpha <- 0.05       # Significance level
shift <- 5          # Location shift added to group2 under the alternative
set.seed(123)       # Random number generator seed number
```

```{r}
# Estimate empirical power for a two-sample Wilcoxon test to detect a location shift applied to group 2.
...
for (outcome in outcomes) {
  ...
  outcome_results <- lapply(group_vars, function(group_col) {
    estimate_empirical_power(data      = df,
                             outcome   = outcome,
                             group_col = group_col,
                             shift     = shift,
                             Nsim      = Nsim,
